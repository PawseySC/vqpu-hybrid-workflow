...
from vqpucommon.vqpuworkflow_deprecated import (
    ...
    async with asyncio.TaskGroup() as tg:
        for i in range(max_num_gpu_launches):
            if np.random.uniform() > 0.5:
                tasks["gpu"].append(
                    tg.create_task(
                        gpu_workflow.with_options(task_runner=task_runners["gpu"])(
                            arguments=arguments
                        )
                    )
                )
                if np.random.uniform() > 0.75:
                    tasks["cpu"].append(
                        tg.create_task(
                            cpu_workflow.with_options(task_runner=task_runners["cpu"])(
                                arguments=arguments
                            )
                        )
                    )
...
async def workflow(
    ...
    for vqpu_id in vqpu_ids:
        ...
        events[f"vqpu_{vqpu_id}_launch"] = EventFile(
            name=f"vqpu_{vqpu_id}_launch", loc="./events/"
        )
        events[f"vqpu_{vqpu_id}_circuits_finished"] = EventFile(
            name=f"vqpu_{vqpu_id}_circuits_finished", loc="./events/"
        )
