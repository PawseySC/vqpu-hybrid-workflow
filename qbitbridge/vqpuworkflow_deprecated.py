...
def test_circuit(remote: str, arguments: str) -> Dict[str, int]:
...
async def launch_vqpu(
    event: EventFile,
    vqpu_id: int,
    path_to_vqpu_script: str = os.path.dirname(os.path.abspath(__file__))
    + "/../qb-vqpu/",
    spinuptime: float = 30,
...
@task(retries=5, retry_delay_seconds=10, timeout_seconds=600)
...
async def task_testcircuitcomplete(
    event: EventFile, circuittime: float, vqpu_id: int
) -> str:
...
async def run_vqpu_test(
    ...
    tasks = [
        asyncio.create_task(
            task_testcircuitcomplete(event, circuittime=circuittime, vqpu_id=vqpu_id)
        ),
        asyncio.create_task(task_walltime(walltime, vqpu_id=vqpu_id)),
    ]
    done, pending = await asyncio.wait(tasks, return_when=asyncio.FIRST_COMPLETED)
...
async def launch_vqpu_workflow(
    ...
    future = await launch_vqpu.submit(
        launch_event, vqpu_id=vqpu_id, arguments=arguments
    )
    await future.result()
    ...
    future = await run_vqpu.submit(
        finish_circuit_event, vqpu_id=vqpu_id, walltime=walltime
    )
    await future.result()
...
async def launch_vqpu_test_workflow(
    ...
    logger = get_run_logger()
    logger.info(
        "Running vqpu launch and shutdown workflow test (no actual vQPU invoked)"
    )
    if try_real_vqpu:
        future = await launch_vqpu.submit(
            launch_event, vqpu_id=vqpu_id, arguments=arguments
        ...
        future = await launch_vqpu_test.submit(
            launch_event, vqpu_id=vqpu_id, arguments=arguments
        )
    await future.result()
    future = await run_vqpu_test.submit(
        finish_circuit_event,
        vqpu_id=vqpu_id,
        walltime=walltime,
...
async def circuits_workflow(
    ...
    results = list()
    for c in circuits:
        result = await run_circuitandpost.fn(
            circuit=c, vqpu_id=vqpu_id, arguments=arguments, remote=remote
        )
...
@task
def circuit_event_clean(events: Dict[str, EventFile], vqpu_ids: List[int]) -> None:
    """Flow for running circuits (and any postprocessing)
    ...
    for c in circuits[key]:
        ...
        if run_silly_compute_test:
            if np.random.uniform() > 0.5:
                await gpu_workflow.with_options(task_runner=task_runners["gpu"])(
                    arguments=arguments
                )
            for i in range(4):
                if np.random.uniform() > 0.2:
                    await cpu_workflow.with_options(task_runner=task_runners["cpu"])(
                        arguments=arguments
                    )
    if circuits_complete:
...
@flow(
    name="Circuits with multi-vQPU, GPU flow",
...
async def cpu_workflow(
    arguments: str = "", date: datetime.datetime = datetime.datetime.now()
) -> None:
